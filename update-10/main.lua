...
function love.draw()
    ...
    -- Promotion dialog
    if Helpers.promotionRequest then
        Visuals.drawPromotionPopup()
    end

    if Helpers.winner then
        Visuals.showVictory(Helpers.winner)
    end
...
function love.mousepressed(x, y, button, istouch, presses)
    -- reproject into virtual coords
    local gx, gy = push:toGame(x, y)

    -- Promotion popup has priority
    if Helpers.promotionRequest then
        if handlePromotionClick(gx, gy) then
            return
        end
    end

    -- Otherwise record click normally (mouse.wasPressed)
...
-- helper to map pixel‐click → which button, if any
function handlePromotionClick(x, y)
    local pr = Helpers.promotionRequest
    local opts = pr.options
    local btnW, btnH, spacing = 100, 30, 10
    local totalW = #opts*btnW + (#opts-1)*spacing
    local startX = OFFSET_X - totalW/2
    local startY = OFFSET_Y - btnH/2

    for i, choice in ipairs(opts) do
        local bx = startX + (i-1)*(btnW + spacing)
        if x >= bx and x <= bx+btnW and y >= startY and y <= startY+btnH then
            Helpers.processPromotionChoice(choice)
            return true
        end
    end
    return false
