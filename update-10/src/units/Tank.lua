...
function Tank:specialDirections()
...
function Tank:moveDirections()
    local dirs = {}
    for _, t in ipairs(self:specialDirections()) do
...
function Tank:computeValidMoves()
    ...
    local raw = self:specialDirections()
    local filtered = {}
...
function Tank:computeValidAttacks()
    local attackable = HexBoard:getAttackableTilesRanged(self.q, self.r, self)
        ...
        -- Quick lookup table to compare
        local attackableMap = {}
        for _, tile in ipairs(attackable) do
            attackableMap[tile.q .. "," .. tile.r] = true
        end
        ...
        local filtered = {}
        for _, tile in ipairs(raw) do
            if attackableMap[tile.q .. "," .. tile.r] then
                table.insert(filtered, tile)
            end
        end

    return self:filterAttackableTiles(filtered)
end
